...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case Nil => List()
    case x :: xs =>
      (x, 1 + xs.count(c => c == x)) :: times(xs.filter(c => c != x))
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortBy(x => x._2).map(x => new Leaf(x._1, x._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match {
      case first :: second :: rest =>
        makeCodeTree(first, second) :: rest
      case _ => trees
    }
  ...
  def until(
    ...
    if (trees.isEmpty) throw new NoSuchElementException
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val trees = until(singleton, combine)(makeOrderedLeafList(times(chars)))
    trees match {
      case codeTree :: Nil =>
        codeTree
      case _ =>
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(t: CodeTree, b: List[Bit], chars: List[Char]): List[Char] = {
      t match {
        case Leaf(c, _) =>
          decodeAcc(tree, b, chars ::: List(c))
        case Fork(left, right, _, _) =>
          b match {
            case List() => chars
            case x :: xs =>
              if (x == 0) decodeAcc(left, xs, chars)
              else decodeAcc(right, xs, chars)
          }
      }
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeAcc(tr: CodeTree, txt: List[Char], bits: List[Bit]): List[Bit] =
      txt match {
        case List() =>
          bits
        case x :: xs =>
          tr match {
            case Leaf(c, _) =>
              if (c == x) encodeAcc(tree, xs, bits)
              else List()
            case Fork(left, right, chars, _) =>
              if (chars.contains(x))
                encodeAcc(left, txt, bits ::: List(0)) ::: encodeAcc(
                  right,
                  txt,
                  bits ::: List(1)
                )
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(t: CodeTree,
                   bits: List[Bit]): CodeTable = {
      t match {
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.map(x => codeBits(table)(x)).flatten
  }
